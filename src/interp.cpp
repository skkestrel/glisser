	...
	namespace interp
	{
			...
			: input(file), binary_hist(_binary_hist), single_precision(_single_precision)
		{
			...
			for (size_t i = 1; i < npl1; i++)
			{
				...
				{
					input >> m1[ind] >> rplsq1[ind] >> aei1[ind].x >> aei1[ind].y >> aei1[ind].z >> oom1[ind].x >> oom1[ind].y >> oom1[ind].z;
				}
			...
			for (size_t i = 0; i < nstep; i++)
			{
				...
				if (use_jacobi_interp)
				{
					...
					for (size_t j = 0; j < pl_alive; j++)
					{
						...
						if (diagnostic_mode)
						{
							if (pl.id()[j] == 3) temp_log << t0 + relative_t << " " << pl.id()[j] << " " << aei_jacobi << " " << oom_jacobi << std::endl;
							if (pl.id()[j] == 4) temp_log << t0 + relative_t << " " << pl.id()[j] << " " << aei_jacobi << " " << oom_jacobi << std::endl;
						}
					}
					...
					// if (diagnostic_mode)
					// {
					// 	temp_log << t0 + relative_t << " " << pl.id()[pl_alive - 1] << pl.r()[pl_alive - 1] << std::endl;
					// 	temp_log << t0 + relative_t << " " << pl.id()[pl_alive - 1] << pl.r()[pl_alive] << std::endl;
					// }
				}
				else
				{
		...
		void Interpolator::next(sr::data::HostPlanetPhaseSpace &pl)
		{
			...
			for (auto id : ids)
			{
				...
				// Omega
				if (reduced_doom[i].x > M_PI)
					reduced_doom[i].x -= 2 * M_PI;
				else if (reduced_doom[i].x < -M_PI)
					reduced_doom[i].x += 2 * M_PI;
