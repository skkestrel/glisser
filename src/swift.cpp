...
#pragma GCC diagnostic pop
					...
					else
					{
						...
						// if a close encounter was logged in tpout
						uint32_t pl_index = istat[2][statindex];
						
						if (pl_index != 0)
						{
							pa.deathtime_map()[pa.id()[i + child.chunk_begin]] = rstat[pl_index + 1][statindex];
							deathflags = static_cast<uint16_t>(planet_id_list[pl_index-2] << 8);
						}
						else
						{
							pa.deathtime_map()[pa.id()[i + child.chunk_begin]] = -1;
							deathflags = 0;
						}
						// std::cout << pl_index << std::endl;
