...
def plot2(time, plotREAL, isAnimation):
    ...
    pl_txt = "reoutput/snapshots/planets_0.txt"
    df_pl0 = opk.snapshot2df(folder1+pl_txt)

    pa_txt = "reoutput/snapshots/particles_0.txt"
    df_pa0 = opk.snapshot2df(folder1+pa_txt)

    a_N = df_pl0['a'].iloc[3]
    klist, jlist, a_rlist = opk.genOuterRes(
        a_N, 49.5, 101, high1=3, high2=50, order_lim=12)

    for t in time:
        fig, [ax3, ax1, ax2] = plt.subplots(3, figsize=(
            13, 9), gridspec_kw={'height_ratios': [1, 4, 1]}, sharex=True)

        pl_txt = "reoutput/snapshots/planets_0.txt".format(t)
        df_pl0 = opk.snapshot2df(folder1+pl_txt)
        ...
        df_pa['isR'] = 0
        for ar in a_rlist:
            df_pa.loc[np.abs(df_pa['a0']-ar) < 0.3, 'isR'] = 1
        print(df_pa)
        ...
        df_pa.loc[df_pa['isR'] == 1, 'color'] = opk.ORANGE
        ...
        if not isAnimation:
            ...
            ax1.text(78, 50, "Kozai\ninside MMR", ha='left',
                     fontsize=16, color=opk.LIGHT_BLUE)
            ax1.text(64.5, 34, "Neptune\nscattering",
                     ha='center', fontsize=16, color=opk.DARK_RED)
...
plot2([0, 36500000000], False, True)
