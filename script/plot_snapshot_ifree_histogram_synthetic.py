...
folder = "/home/yhuang/GLISSER/glisser/fast/cold_output/out-ivp-ifree-low-N1/reoutput/"
...
pa_txt = "snapshots/particles_{0:d}.txt".format(time[0])
df_pa0 = pd.read_csv(folder+pa_txt, names=['id', 'a', 'e', 'inc', 'Omega', 'omega', 'f'], delimiter=' ').set_index('id')
# pa_txt = "snapshots/particles_{0:d}.txt".format(time[1])
# df_pa1 = pd.read_csv(folder+pa_txt, names=['id', 'a', 'e', 'inc', 'Omega', 'omega', 'f'], delimiter=' ').set_index('id')
# pa_txt = "snapshots/particles_{0:d}.txt".format(time[2])
# df_pa2 = pd.read_csv(folder+pa_txt, names=['id', 'a', 'e', 'inc', 'Omega', 'omega', 'f'], delimiter=' ').set_index('id')
...
df_pa_stable = df_pa3[df_pa3['lifetime'] > 99.9].copy()
...
plt.savefig(output_folder+"ifree_cold_low_histogram (group 1)_synthetic.jpg".format(idx=idx),dpi=400)
