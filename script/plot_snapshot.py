...
folder = "/home/yhuang/GLISSER/glisser/fast/rogue_output/out-JSUNR-Resonant/"
...
for t in time:
    ...
    df_pa = pd.read_csv(folder+pa_txt, names=['id', 'a', 'e', 'inc', 'Omega', 'omega', 'f', 'If'], delimiter=' ')
    print(df_pa)

    fig, [ax1, ax2] = plt.subplots(2, figsize=(9,10))
    df_pl['q'] = df_pl['a'] * (1 - df_pl['e'])
    ax1.scatter(df_pl['a'], df_pl['q'], alpha=0.8, s=100, color=red, marker='x',lw=2, zorder=2)
    ax2.scatter(df_pl['a'], np.rad2deg(df_pl['inc']), alpha=0.8, s=100, color=red, marker='x',lw=2, zorder=2)

    df_pa['q'] = df_pa['a'] * (1 - df_pa['e'])
    par_size = 2
    ax1.scatter(df_pa['a'], df_pa['q'], alpha=0.4, s=par_size, edgecolors='none', facecolors='C0')
    ax2.scatter(df_pa['a'], np.rad2deg(df_pa['If']), alpha=0.4, s=par_size, edgecolors='none', facecolors='C0')
