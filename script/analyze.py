...
if final.shape[0] < 20001:
    ...
    plt.figure()
    #plt.title('survival time given initial conditions')
    plt.scatter(initial[alive, 2] / math.pi * 180, final[alive, 6] / 365.25e6, s=1, c=cFromFlags(final[alive, 6]))
    ...
    plt.figure()
    #plt.title('survival time given initial conditions')
    plt.scatter(initial[alive, 1], final[alive, 6] / 365.25e6, s=1, c=cFromFlags(final[alive, 6]))
    plt.ylim([0.00001, 4500])
    plt.xlabel('e_i')
    plt.yscale('log')
    plt.ylabel('survival time (MYr)')
