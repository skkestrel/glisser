...
def plotHist(target_pa, target_pl, outputEnc):
    ...
    fig, axes = plt.subplots(3, figsize=(
        8, 7.5), gridspec_kw={'height_ratios': [3, 3, 1]}, sharex=True)
    ax1, ax2, ax3 = axes[0], axes[1], axes[2]
    par_size = 1
    alpha = 1

    ax1.scatter(df_pa['time'], df_pa['a'], alpha=1,
                s=par_size*3.5, facecolors=df_pa['color'], edgecolors='none', rasterized=True)
    # ax1.scatter(df_pl['time'], df_pl['a'], alpha=1,
    #             s=par_size*3.5, facecolors=opk.DARK_RED, edgecolors='none', rasterized=True)            
    # axE = ax1.twinx()
    ...
    ax2.scatter(df_pa['time'], df_pa['q'], alpha=alpha,
                s=par_size, rasterized=True)
    ax5 = ax2.twinx()
    ax5.scatter(df_pa['time'], df_pa['inc'],
                alpha=0.5, s=par_size, color=opk.DARK_RED, rasterized=True)


    ax3.scatter(df_pa['time'], opk.wrapTo180((df_pa['omega']+df_pa['Omega'])-(df_pl['omega']+df_pl['Omega'])),
                alpha=alpha, s=par_size, color=df_pa['color'],  rasterized=True)


    # ax4.scatter(df_pa['time'], opk.wrapTo360(df_pa['omega']),
    #             alpha=alpha, s=par_size, rasterized=True)
    # axO = ax4.twinx()
    # axO.scatter(df_pa['time'], opk.wrapTo360(df_pa['Omega']),
    #             alpha=0.2, s=par_size, color=opk.DARK_RED, rasterized=True)
    ...
    ax2.plot([t0, t1], [40, 40], color='gray',
    ...
    # ax1.set_title("With Rogue", fontsize=22)
    ax1.set_ylabel("a (au)")
    # ax1.set_yscale('log')
    # axE.set_ylabel("Encounter dist (au)", color=opk.DARK_RED)
    # axE.set_ylim([0.1, 4])
    # axE.set_yscale('log')

    ax2.set_ylabel("q (au)", color=opk.BLUE)
    ax5.set_ylabel("I (deg)", color=opk.DARK_RED)
    ax3.set_ylabel(r"$\Delta\varpi$")
    ax3.set_ylim([-180, 180])
    # ax4.set_ylabel(r"$\omega$", color=opk.BLUE)
    ...
    # fig.align_ylabels([ax5])

    for ax in [ax5]:
        ax.tick_params(axis='y', colors=opk.DARK_RED)

    # for ax in [ax3]:
    #     ax.tick_params(axis='y', colors=opk.GRAY)

    plt.savefig(os.path.join(output_folder, "particle_{idx:d}.jpg".format(idx=target_pa)), dpi=200)
    # plt.savefig("particle_{idx:d}.pdf".format(idx=target_pa), dpi=200)
...
for target_pa in [1740,13363,15672,16673,16897,23747,29595,45713,59885,60112, \
68266,76620,82132,103253,123108,125737,144969,148436,181953,183129, \
188996,193624,199529,199762,200762,209280,210396,215662,217404,223567, \
226328,227833,232536,240967,243215,244650,248544,250587,261307,261725, \
268840,271523,273024,277401,292936,293080,309641,326940,328462,330530, \
347932,353062,376018,379527,380074,396106,396506]:
    plotHist(target_pa, 5, True)
