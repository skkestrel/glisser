...
def plotHistRes(target_pa, target_pl, outputEnc):
    ...
    fig, axes = plt.subplots(4, figsize=(
        8, 7.5), gridspec_kw={'height_ratios': [3, 1, 2, 1]}, sharex=True)
    ...
    if code > -3:
        ...
        ax3.scatter(df_pa['time'], opk.wrapTo360(phi),
                    alpha=0.5, s=par_size*3.5, color=df_pa['color'], facecolors=df_pa['color'], edgecolors='none', rasterized=True)
        loc = opk.resonanceLocationBYA(np.mean(df_pl['a']), k, j)
        ax1.plot([t0, t1], [loc, loc],
    ...
    plt.savefig("particle_{idx:d}_norogue.jpg".format(idx=target_pa), dpi=200)
    plt.savefig("particle_{idx:d}_norogue.pdf".format(idx=target_pa), dpi=200)
