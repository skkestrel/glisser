...
nparts = [1, 10, 100, 500, 1000, 3000, 5000, 10000, 13312, 13313, 15000, 17500, 20000, 24000, 26626, 26627, 39936, 53248, 66560, 79184, 93184, 106496, 119808, 133120]
...
if mode == 0:
	...
	for T in reversed(tbsizes * 3):
		...
		maxtime = run()
		with open('timeblock-prof.csv', 'a') as profout:
	...
	for T in nparts * 3:
		...
		maxtime = run()
		with open('particle-prof.csv', 'a') as profout:
	...
	for T in npls * 3:
		...
		maxtime = run()
		with open('planet-prof.csv', 'a') as profout:
