...
folder = "/home/yhuang/GLISSER/glisser/benchmark/out-N-scattering-3/"
...
def plotSnapshotAQI(time, idx, plotReal=False, plotCold=400000):
    ...
    for t in time:
        ...
        # pa_txt = "reoutput/snapshots/particles_0.txt"
        ...
        gs = GridSpec(2, 2, width_ratios=[1, 1.2], height_ratios=[3, 1])
        ...
        df_pa.loc[df_highq, 'size'] = par_size*15
        df_pa.loc[df_highq, 'color'] = opk.LIGHT_BLUE
        df_pa.loc[df_highq, 'alpha'] = np.clip(alpha*2, a_min=0, a_max=1)
        ...
        # filter out unplotted particles
        df_pa = df_pa[df_pa['a'] > aleft]
        for ax in [ax3, ax1]:
            ax.scatter(df_pl['a'], df_pl['q'], alpha=1, s=100,
                       color=df_pl['color'], marker='x', lw=3, zorder=20, rasterized=True)
            ax.scatter(df_pa['a'], df_pa['q'], alpha=df_pa['alpha'],
        ...
        for ax in [ax4, ax2]:
            ...
            ax.scatter(df_pa['a'], df_pa['inc'], alpha=df_pa['alpha'],
        ...
        if plotReal:
            ...
            # print(df_real.sort_values(by='sigma_a')['sigma_a'])
            for ax in [ax3, ax1]:
                ax.scatter(df_real['a_bary'], df_real['q_bary'], alpha=df_real['alpha'], s=df_real['size'],
                           color=opk.DARK_RED, marker='x', lw=1.5, zorder=2)
            for ax in [ax4, ax2]:
                ax.scatter(df_real['a_bary'], np.rad2deg(df_real['inc_bary']), alpha=df_real['alpha'], s=df_real['size'],
                           color=opk.DARK_RED, marker='x', lw=1.5, zorder=2)
        ...
        yticks = np.arange(20, round(q1/10+1)*10, 10)
        yticks[-1] = 100
        yticks[0] = 25
        for ax in [ax1, ax3]:
        ...
        fig.tight_layout()
        ax2.xaxis.set_label_coords(0.1, -0.35)
...
# time = np.arange(t0, 3*37000000001, time_step)

# idx = 31500000000/time_step
time = [36500000000]
plotSnapshotAQI(time, idx, plotReal=True, plotCold=400000)
# time = [t0]
