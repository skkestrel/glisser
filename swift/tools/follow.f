c converts binary file to ascii file
	...
	integer iflgchk,iu,nleft,i,id
        ...
        integer io_read_hdr_r,io_read_line_r, ialpha
...
c Get data for the run and the test particles
 ...
 1      continue
     ...
     &			,capm,xh(i),yh(i),zh(i),vxh(i),vyh(i),vzh(i))
		   ...
		   if (e .lt. 1.d0) then
			 ...
			 ialpha = 1
		      ...
		      call orbel_el2xv(gm,ialpha,a,e,inc,capom,omega
     &			,capm,xht(i),yht(i),zht(i),vxht(i),vyht(i),
     &			vzht(i))
		      peri = dsqrt(xht(i)**2 + yht(i)**2 + zht(i)**2)
		      write(7,1002) t,a,e,inc,capom,omega,capm,peri,apo
		   end if
                   tmax = t
                   write(8,1001) t,elh,elk,elp,elq
                endif
 ...
 1002	format(1x,e15.7,1x,f10.4,1x,g8.2,3(1x,f9.4),2(1x,g11.4))
