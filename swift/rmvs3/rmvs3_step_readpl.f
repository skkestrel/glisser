...
c Date:    7/10/96
c Last revision: modified on Oct 20 by Morby to work in the
c case where the planetary orbits are read from a file.
c swift_rmvs3_readpl.f is the main code.


      subroutine rmvs3_step_readpl(i1st,time,nbod,ntp,mass,
     &     j2rp2,j4rp4,xh,yh,zh,vxh,vyh,vzh,xht,yht,zht,vxht,
     &     vyht,vzht,istat,rstat,dt,xend,yend,zend,vxend,vyend,vzend)	
...
c...  Internals
      integer i1stpl,i1sto,icflg,i,j,np
...
c...  are there any encounters?
      rts = RHSCALE*RHSCALE
      call rmvs3_chk_readpl(nbod,ntp,mass,xh,yh,zh,vxh,vyh,vzh,xht,yht,
...
c...  Now do the interpolation for intermediate steps
      call rmvs3_interp_readpl(nbod,xbeg,ybeg,zbeg,vxbeg,vybeg,vzbeg,
     &     xend,yend,zend,vxend,vyend,vzend,dt,mass(1),NTENC,
     &     xtmp,ytmp,ztmp,vxtmp,vytmp,vztmp)

c...  do the integration
      call rmvs3_step_out_readpl(i1st,nbod,ntp,mass,j2rp2,j4rp4,
