...
bool load_data(HostData& hd, std::string plin, std::string icsin, size_t tbsize, size_t max_particle, bool readmomenta)
{
	...
	auto part = thrust::partition(hd.particles.begin(), hd.particles.begin() + hd.particles.n, HostParticleAlivePredicate());
	hd.particles.n_alive = part - hd.particles.begin();
